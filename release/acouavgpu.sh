...
EXEACO_GPU=$ACOUAV_HOME/acouav_gpu_noaging_nobroker
EXEACO_GPU_AGING=$ACOUAV_HOME/acouav_gpu_aging_nobroker
...
EXEC_ACOUAV() {
	...
	do
		...
		do
			...
			if [ -f "$EXEACO_GPU_AGING" ]; then
				$EXEACO_GPU_AGING -m $ants -s $iter -x $minp -y $maxp -o $optimum -t $MAXTIME -i $TSPDIR/$tsp.tsp -p $OUTDIR/$tsp.txt -a 1.0 -b 2.0 -r 0.5
			fi
